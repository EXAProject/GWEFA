...
if (!isset($_GET['dir'])) {
    ...
    if (QUARANTINE_USE_FLAG) {
        ...
        echo '<tr><th colspan=8 style="padding: 12px;font-size: 16px;">' . __('folder08') . '</th></tr><tr>' . "\n";
        if ($i == 0) { echo '</tr>'; }
        foreach ($dates as $date) {
            ...
            echo '<td style="padding: 12px;font-size: 12px;"><a href="quarantine.php?token=' . $_SESSION['token'] . '&amp;dir=' . $date . '">' . translateQuarantineDate(
                    $date,
                    DATE_FORMAT
                ) .  '</a></td>' . "\n";
            echo '<td style="padding: 12px;font-size: 12px;">' . $rowstr . '</a></td>' . "\n";
